...
{
    int addrtype;
    return g_pBoard->GetWordView(addr, m_processor->IsHaltMode(), false, &addrtype);
}

uchar QEmulatorProcessor::readByte(ushort addr)
{
    int addrtype;
    ushort word = g_pBoard->GetWordView(addr, m_processor->IsHaltMode(), false, &addrtype);
...
QScriptValue QEmulatorProcessor::disassemble(ushort addr)
{
    ...
    {
        int addrtype;
        buffer[i] = g_pBoard->GetWordView(current, m_processor->IsHaltMode(), false, &addrtype);
        current += 2;
    }
