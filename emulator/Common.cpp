...
bool AssertFailedLine(const char * lpszFileName, int nLine)
{
    ...
    _snprintf(buffer, 360,
            ...
            lpszFileName, nLine);
    int result = QMessageBox::question(nullptr, QT_TRANSLATE_NOOP("Common", "NEON Back to Life"),
...
{
    QMessageBox::information(nullptr, QT_TRANSLATE_NOOP("Common", "NEON Back to Life"),
            sMessage, QMessageBox::Ok);
}

void AlertWarning(const QString &sMessage)
{
    QMessageBox::warning(nullptr, QT_TRANSLATE_NOOP("Common", "NEON Back to Life"),
            sMessage, QMessageBox::Ok);
}
